// Admin-only writes to the single-row `alignment_tuning` table.
//
// Reads stay public (anyone can SELECT — /build has no login wall and needs the current
// alignment to render). Writes do not: this route is the only path that can UPDATE the row, and
// it uses the service-role client (bypasses RLS) only after proving the caller is an admin via
...
export async function PUT(request: Request) {
  ...
  const { error } = await supabase.from('alignment_tuning').update({ data: body.data }).eq('id', true);
