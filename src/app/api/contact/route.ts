// Public submission for the site's "Contact us" form. Same reasoning as
// /api/gbb/route.ts: the anon key has no insert policy on contact_messages,
// so this is the only path in, and it's rate-limited by IP so a script can't
// flood the table.
...
export async function POST(request: Request) {
  ...
  const rateLimit = await checkRateLimit(supabase, `contact:${ip}`, { limit: 5, windowSeconds: 10 * 60 });
