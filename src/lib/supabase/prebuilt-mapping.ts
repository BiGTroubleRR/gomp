...
export function rowToBuild(row: PrebuiltRow): Build {
  return {
    ...
    psu: row.psu,
    case: row.case,
    moboId: row.mobo_id ?? undefined,
    cpuId: row.cpu_id ?? undefined,
    coolerId: row.cooler_id ?? undefined,
    ramId: row.ram_id ?? undefined,
    gpuId: row.gpu_id ?? undefined,
    storageId: row.storage_id ?? undefined,
    psuId: row.psu_id ?? undefined,
...
export function buildToRow(build: Build, sortOrder: number): PrebuiltInsert {
  return {
    ...
    psu: build.psu,
    case: build.case,
    mobo_id: build.moboId ?? null,
    cpu_id: build.cpuId ?? null,
    cooler_id: build.coolerId ?? null,
    ram_id: build.ramId ?? null,
    gpu_id: build.gpuId ?? null,
    storage_id: build.storageId ?? null,
    psu_id: build.psuId ?? null,
    case_id: build.caseId ?? null,
